...
def scrape_ap3():
    ...
    path = functions.create_path("ap3")
    ...
    for w_pdf in wanted_pdfs:
        ...
        while not link_append:
            ...
            dec_pat = ".+" + w_pdf + ".+" + "[december0-9\-]+" + str(year) + "\.pdf"
            june_pat = ".+" + w_pdf + ".+" + "[june0-9\-]+" + str(year) + "\.pdf"
    ...
    with pdfplumber.open(pdf_paths[0]) as pdf:
        ...
        for match in matches:
            ...
            append_list = [shareholder, issuer, sectype, isin, report_date, value, currency, ticker, shares, url]
            #Append entry
            entries.append(append_list)


        #Create dataframe according to IDI schema, and export
        df_swedish = pd.DataFrame(entries, columns = ["Shareholder - Name", "Issuer - Name", "Security - Type", "Security - ISIN", "Security - Report Date", "Security - Market Value", "Security - Market Value - Currency Code", "Stock - Ticker", "Stock - Number of Shares", "Data Source URL"])
        functions.export_df(df_swedish, "ap3_swedish", path)
    ...
    with pdfplumber.open(pdf_paths[1]) as pdf:
        ...
        foreign_pattern = re.compile("(?P<issuer>[A-Z\-\./& ]+) (?P<ticker>[A-Z\d/]+ [A-Z]{2}) (?P<sectype>Equity|Fund EQ) (?P<currency>[A-Z]{3}) (?P<shares>\d{1,3}(?:,\d{1,3})*\.\d{2}) (?P<value>\d{1,3}(?:,\d{1,3})*) (?P<isin>[A-Z0-9]+)?")
        ...
        #Create dataframe according to IDI schema, and export
        df_foreign = pd.DataFrame(entries, columns = ["Shareholder - Name", "Issuer - Name", "Security - Type", "Security - ISIN", "Security - Report Date", "Security - Market Value", "Security - Market Value - Currency Code", "Stock - Ticker", "Stock - Number of Shares", "Data Source URL"])
        functions.export_df(df_foreign, "ap3_foreign", path) 
    ...
    with pdfplumber.open(pdf_paths[2]) as pdf:
        ...
        fixed_pattern = re.compile("(?P<issuer>^[A-Z][A-Za-z\-\./& ]+) (?P<sectype>Corporates|Governments & Sovereigns|Mortgages & Agencies|FUND FI|Bond) (?P<value>\d{1,3}(?:,\d{1,3})*)$", re.MULTILINE)
        ...
        for match in matches:
            ...
            #Order variables in 1 entry
            append_list = [shareholder, issuer, sectype, report_date, value, url]
            #Append entry
            entries.append(append_list)
        
        
        #Create dataframe according to IDI schema, and export
        df_fixed = pd.DataFrame(entries, columns = ["Shareholder - Name", "Issuer - Name", "Security - Type", "Security - Report Date", "Security - Market Value", "Data Source URL"])
        functions.export_df(df_fixed, "ap3_fixed", path) 
    ...
    with pdfplumber.open(pdf_paths[3]) as pdf:
        ...
        private_pattern = re.compile("(?P<issuer>[A-Za-z\-\.\d\| /&,()]+) (?P<currency>[A-Z]{3}) (?P<value>\d{2}) (?P<vintage_year>\d{4})")
        ...
        for match in matches:
            ...
            append_list = [shareholder, issuer, report_date, value, multiplier, currency, vintage_year, url]
            #Append entry
            entries.append(append_list)


        #Create dataframe according to IDI schema, and export
        df_private = pd.DataFrame(entries, columns = ["Shareholder - Name", "Issuer - Name", "Security - Report Date", "Security - Market Value", "Security - Market Value - Multiplier", "Security - Market Value - Currency Code", "Private Equity - Vintage year", "Data Source URL"])
        functions.export_df(df_private, "ap3_private", path)
