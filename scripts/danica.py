...
def scrape_danica():
    #Create path
    path = functions.create_path("danica")
    ...
    with sync_playwright() as p:
        ...
        while bool(pdf_url) == False:
            try:
                pdf_url = years_to_try[i].locator('table tbody tr td:has-text("Aktiebog") + td a').get_attribute("href", timeout = 1000)
            except:
                i = i+1
                continue
    ...
    #download pdf
    pdf_path = functions.download_file(r, path/"raw_danica.pdf", path)
    #print(r.status_code)
        ...
        if text:
            ...
            for line in lines: 
                ...
                if "KAPITALANDELE" not in line:
                    ...
                    if match:
                        data.append({"Shareholder - Name": "Danica Pension", "Issuer - Name": match.groups()[0], "Security - Report Date": date, "Security - Market Value - Amount": f"{match.groups()[1]}", "Security - Market Value - Multiplier":"x1_000_000", "Security - Market Value - Currency Code":"DKK", "Report Date URL": f"https://danica.dk/regnskaber/aarsrapporter{pdf_url}"})


    #makes a dataframe out of the data list, exports it to csv just for testing purposes for now#
    df = pd.DataFrame(data)
    functions.export_df(df, "danica", path)
