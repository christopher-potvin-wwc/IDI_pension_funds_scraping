...
#External Modules for tesseract
import pytesseract
...
if __name__ != "__main__":
    ...
    log_type = "a" #Append to logs if run from main
else:
    import functions
    log_type = "w" #Write new log if run locally




'''Main function'''
def scrape_sampension():

    #Set up Logging
    parent_dir = Path(__file__).parent.parent 
    log_path = parent_dir/"log.log"
    logging.basicConfig(filename=log_path, level=logging.INFO, filemode=log_type, format="%(asctime)s - %(message)s")
    ...
    if osys == "Windows":
        ...
        elif os.path.isfile(rf"C:\Users\{user}\AppData\Local\Programs\Tesseract-OCR"):
            pytesseract.pytesseract.tesseract_cmd = rf"C:\Users\{user}\AppData\Local\Programs\Tesseract-OCR"
        else:
    ...
    #Untested - I don't have a mac to test this with
        ...
        if os.path.isfile("/Applications/Tesseract-OCR/tesseract.exe"):
            pytesseract.pytesseract.tesseract_cmd = "/Applications/Tesseract-OCR/tesseract.exe"
        elif os.path.isfile(f"/Users/{user}/Applications/Tesseract-OCR/tesseract.exe"):
            pytesseract.pytesseract.tesseract_cmd = f"/Users/{user}/Applications/Tesseract-OCR/tesseract.exe"
        else:
            logging.info("Sampension -Failed to find tesseract path")
            return
    ...
    #Untested - I don't have linux to test this with
        ...
        if os.path.isfile("/usr/local/bin/Tesseract-OCR/tesseract.exe"):
            pytesseract.pytesseract.tesseract_cmd  = "/usr/local/bin/Tesseract-OCR/tesseract.exe"
        elif os.path.isfile("/usr/bin/Tesseract-OCR/tesseract.exe"):
            pytesseract.pytesseract.tesseract_cmd  = "/usr/bin/Tesseract-OCR/tesseract.exe"
        elif os.path.isfile(f"/home/{user}/.local/share/Tesseract-OCR/tesseract.exe"):
            pytesseract.pytesseract.tesseract_cmd  = f"/home/{user}/.local/share/Tesseract-OCR/tesseract.exe"
        else:
            logging.info("Sampension - Failed to find tesseract path")
            return
    ...
    #Create path
    path = functions.create_path("sampension")
    #URL to reports
    ...
    pdf_path = functions.download_file(req, "raw_sampension.pdf", path)
    ...
    for line in page_text:
        ...
        #Search for date
        m = re.search("(?P<date>\d+)\. (?P<month>[A-Za-z]+) (?P<year>\d{4})", line)
    ...
    value_isin_pattern = re.compile(r"(?P<value>\d[\d\. ]+) (?P<isin>[A-Z\d ]{12,})")
    ...
    for page_num, page in enumerate(pdf.pages):
        ...
        try:
            ...
            im = pdf.pages[page_num].to_image(resolution=144).save(path/"sampension.png")
            #Use tesseract to get text from said image
            string_cheese = pytesseract.image_to_string(Image.open(path/"sampension.png"))
        except:
        ...
        if string_cheese:
            ...
            string_cheese = re.sub("’|£|\|", "", string_cheese)
    ...
    try:
        ...
        logging.info("Sampension - DF column lengths do not match. Manual review needed.")
        return
    #Export as TSV
    functions.export_df(df, "sampension", path)
