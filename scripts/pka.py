...
def scrape_pka():
    ...
    path = functions.create_path("pka")
    ...
    link_button = page.get_by_role('link', name="Se beholdningslisten", exact=True) 

    #Get PDF
    pdf_path = functions.get_pdf("pka", page, link_button, browser, path)
    playwright.stop()
    ...
    pattern = re.compile(r'^(.*?)\s+([A-Z]{2}[A-Z0-9]{10})\s+([\d.,]+)\s+([\d.,]+\s*%)$')
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        with open(path/"pka.tsv", "w", newline='', encoding='utf-8') as f:
            ...
            writer.writerow(["Shareholder - Name", "Issuer - Name", "Security - Report Date", "Security - Market Value - Amount", "Security - Market Value - Currency Code", "Security - ISIN", "Stock - Percent Ownership", "Data Source URL"])
                ...
                if text:
                    ...
                    for line in lines:
                        ...
                        if match:
                            ...
                            issuer, isin, value, percent_ownership = match.groups()
                            #Write to CSV
                            writer.writerow([shareholder, issuer, report_date, value, currency_code, isin, percent_ownership, url]) 
