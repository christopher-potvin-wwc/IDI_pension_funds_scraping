'''Scrapes AMF Pension, manager for over four million pension holders, based in Sweden. Scraper finds most recent pdf and downloads using requests. Then, it begins looping through each page of the document, using a combination of keywords, page cropping, font type, and text size to filter out desired entries. Additionally, the security type is stored using a match statement and applied to following entries until a new security type is listed, or the page ends. No manual steps needed unless the URL format or Pdf format changes. Note: This pdf has a lot of edge cases, so the likelyhood that this one needs to be tweaked or redone in the future is high.'''
#Python modules
import re
import datetime
