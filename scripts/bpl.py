...
#Import custom module locally, regardless of the script run
...
def scrape_bpl():
    ...
    path = functions.create_path("bpl")
    #Website url
    ...
    #Expand list
    list_button = page.get_by_role('button', name="Verslagen en rapportages") #Reports
    list_button.click()

    #Save button that leads to pdf preview
    link_button = page.get_by_role('link', name="Beleggingsoverzicht") #Investment overview

    #Download pdf (returns pdf path)
    pdf_path = functions.get_pdf("bpl", page, link_button, browser, path)
    ...
    for m in matches:
        ...
        entry = [shareholder, issuer, report_date, value, multiplier, currency, stock, url]
        #Append entry
    ...
    df = pd.DataFrame(entries, columns=["Shareholder - Name", "Issuer - Name", "Security - Report Date", "Security - Market Value - Amount", "Security - Market Value - Multiplier", "Security - Market Value - Currency Code", "Stock - Percent Ownership", "Data Source URL"])
    functions.export_df(df, "bpl", path)
