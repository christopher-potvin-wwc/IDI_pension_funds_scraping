...
def scrape_ap7():
    ...
    path = functions.create_path("ap7")
    ...
    text = re.findall(re.compile("Securities Share Market value Position(.+)"), text)[0]
    ...
    pattern = re.compile("(?P<issuer>(?:[A-Za-z]|\d[A-Z])[A-Za-z\d /&\-\.)(]+) (?P<ownership>[\d,]+\%)[\d ]+Market value (?P<value>\d+) Position (?P<shares>\d+) Currency (?P<currency>[A-Z]{3}) Exchange rate (?P<ex_rate>[\d\.]+) Price/interest (?P<P_I>[\d\.]+(?:E-\d)?) Securities F_Market Value - (?P<sectype>[A-Za-z ]+) Datum: (?P<report_date>\d{4}-\d{1,2}-\d{1,2})")
    ...
    for match in matches:
        ...
        entry = ["AP7", issuer, sectype, report_date, value, currency, ex_rate, shares, ownership, url]
        #Append
    ...
    df = pd.DataFrame(entries, columns = ["Shareholder - Name", "Issuer - Name", "Security - Type", "Security - Report Date", "Security - Market Value", "Security - Market Value - Currency Code", "Stock - Exchange", "Stock - Number of Shares", "Stock - Percent Ownership", "Data Source URL"])
    #Export as TSV at path
    functions.export_df(df, "ap7", path)
