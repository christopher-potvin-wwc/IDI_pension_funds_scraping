...
def scrape_railov():
    ...
    needed = ["Aandelen ontwikkelde markten", "Aandelen opkomende markten", "Bedrijfsobligaties investment grade", "High Yield obligaties"]
    ...
    for i in range(len(contents_df)):
        if contents_df["necessary"][i] == True:
            page_range.append(list(range(int(contents_df["page"][i]), (int(contents_df["page"][i+1])))))
    ...
    df = pd.DataFrame(tabs, columns=["Issuer - Name", "Security - Market Value - Amount", "Percent of Portfolio, to be Deleted"])

    #adding the remaining columns and constants
    df["Security - Report Date"] = functions.get_pdf_date(pdf)
    ...
    df = df[["Shareholder - Name", "Issuer - Name", "Security - Report Date", "Security - Market Value - Amount", "Security - Market Value - Currency Code", "Data Source URL"]]
    ...
    #export dataframe!
    functions.export_df(df, "railov", path)
